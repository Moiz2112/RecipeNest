...
function NewIngredientDialog({ ingredientList, updateIngredientList }: NewIngredientDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error) {
        setMessage(error || 'Unable to add ingredient. Please try again.');
        setIngredientName('');
        setSuggestions([]);
        return;
      }
      ...
      if (responseMessage === 'Success') {
        const possibleSuggestions = response.suggested ? response.suggested.join(', ') : '';
        ...
        setMessage(`${ingredientName} is invalid. Please try a different ingredient.`);
        setIngredientName('');
      } else if (responseMessage === 'Error: This ingredient already exists') {
        setMessage('This ingredient already exists in the database.');
        setIngredientName('');
        setSuggestions([]);
      } else if (responseMessage === 'Error') {
        setMessage(response.error || 'Unable to validate ingredient. Please try again.');
        setIngredientName('');
        setSuggestions([]);
      } else {
        setMessage(`${responseMessage || 'An error occurred with validation... please try again'}`);
        setIngredientName('');
        setSuggestions([]);
      }
    } catch (error) {
      console.error('Error adding ingredient:', error);
      setMessage('Unable to add ingredient. Please try again.');
      setSuggestions([]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <>
      ...
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="relative z-modal">
